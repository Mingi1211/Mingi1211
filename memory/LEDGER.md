...
| 2026-09-19 | 사실 | **로봇학부 학수번호 체계 = `I050-학년-과목번호-분반`.** `I050`=로봇학부, 두 번째 자리가 개설학년 | 수강신청 자료집 화면 |
| 2026-09-19 | 사실 | 로봇학부 개설 35과목을 `db/seed-robot-courses.sql` 에 입력 (1학기 18 · 2학기 17). 전공필수 확인분: 로봇운동학 · AI로봇실험3 · 로봇학실험3 | 사용자 제공 화면 3장 |
| 2026-09-19 | 사실 | 로봇학실험 **1·3 = 1학기, 2·4 = 2학기 전공필수**. 3 외에는 학수번호·학점·학년 **[미확인]** — 뼈대만 넣고 표시해 둠 | 사용자 진술 |
| 2026-09-19 | [정정] | 사용자가 "1번 이미지=1학기, 2·3번=2학기"라고 했으나 **반대였다.** 근거: 회로이론/자동제어 1·2 번호, 로봇게임세미나 26-1/26-2, 로봇학실험3 위치, 현재 수강 중인 로봇제어·로실4 위치 — 넷이 모두 일치 | 이미지 대조 |
| 2026-09-19 | 결정 | `course` 에 **교수 연락처를 넣지 않는다.** 담당 교수명만 `note` 에 남긴다 | 개인정보 규칙 |
| 2026-09-19 | 결정 | 시드 SQL 은 `on conflict do update` 로 **재실행 안전하게** 쓴다. `course` 의 유일키는 `(dept_code, name)` | `db/seed-robot-courses.sql`, 2회 실행 검증 |

| 2026-09-19 | [정정] | 학기 배정은 **Claude 가 맞았다** — 사용자가 반대로 말했던 것. 1번 화면=2학기, 2·3번=1학기 (사용자 확인) | 사용자 확인 |
| 2026-09-19 | 사실 | **로봇학실험 1~4 는 네 과목 모두 운영이 같고 분반이 달라도 같다**: 프로젝트 15주차(기말 대체) · 중간 필기 1회 · 과제는 시험 주차 뺀 매주 · 퀴즈 없음 | 사용자 진술 |
| 2026-09-19 | 결정 | 로봇학실험 4건은 **폼을 거치지 않고 `report` 에 `source='seed'` 로 직접 넣는다** (`db/seed-lab-reports.sql`). 전공필수라 전원이 듣고 운영이 고정 → 폼으로 받으면 분반 수만큼 중복만 쌓인다 | 사용자 지시 |
| 2026-09-19 | 결정 | `course` 에 **`professors text[]` 컬럼 추가.** 담당 교수가 학수번호보다 중요하고, W3 폼의 교수 선택지가 여기서 나와야 교수명 표기도 안 갈라진다. 분반 여럿이면 배열에 여럿 | 사용자 지시 |
| 2026-09-19 | 사실 | `course.code`(학수번호)는 **검색·식별에 쓰지 않는다.** `typical_year` 를 뽑은 근거로만 남긴다 | 사용자 지시 |
| 2026-09-19 | 노하우 | 기존 DB 를 reset 없이 따라오게 하려면 시드 파일 맨 위에 `alter table ... add column if not exists` 를 둔다. 스키마 변경을 사용자가 다시 Run 하지 않아도 된다 | `db/seed-robot-courses.sql` |
| 2026-09-19 | 미확인 | 로봇학실험 15주차 프로젝트가 **팀인지 개인인지** — `project_type` 을 null 로 두었다 |
| 2026-09-19 | 미확인 | 로봇학실험 1·2·4 의 학수번호·담당교수. 개설학년(2·2·3)은 3이 3학년인 것에 맞춘 **추정** |

| 2026-09-19 | 사실 | 로봇학실험 15주차 프로젝트는 **팀 프로젝트** (`project_type='team'`) | 사용자 확인 |
| 2026-09-19 | [정정] | 로봇학실험 배치 확정 — **1 = 2-1, 2 = 2-2, 3 = 3-1, 4 = 3-2.** 앞서 [추정]으로 둔 개설학년이 맞았다 | 사용자 확인 |
| 2026-09-19 | 결정 | 로봇학실험 4과목은 **학수번호·담당교수를 저장하지 않는다.** 전 분반 운영이 같고, **이 서비스는 과제·프로젝트 같은 정량 정보로 조합을 계산하지 교수 개인의 특성·재량을 입력으로 쓰지 않는다** | 사용자 지시 |
| 2026-09-19 | 원칙 | 위 문장은 로봇학실험만의 이야기가 아니다. **교수 정보는 "어느 개설분을 말하는지" 식별용이지 부담 계산의 입력이 아니다.** 발표 방어의 "교수 평가 0" 축과 같은 선 | 사용자 지시에서 |

| 2026-09-19 | 사실 | **학수번호 접두사 = 학부.** `I050` 로봇학부 · `I040` 정보융합학부 · `I030` 소프트웨어학부 | 수강신청 자료집 화면 |
| 2026-09-19 | 사실 | 교과과정 **102과목** 입력 완료 — 소프트웨어 36 · 로봇 35 · 정보융합 31. 세 학부 화면 모두 **전공선택만** 담고 있었고 전공필수는 잘려 있었다 | `db/seed-*-courses.sql` |
| 2026-09-19 | 결정 | **분반은 `course` 행을 만들지 않는다.** 과목명+담당교수가 같으면 운영이 같은 한 과목. 분반별 교수가 다르면 `professors` 배열에 모두 넣는다. 교수별 부담 차이는 `report.course_key` 가 갈라 준다 | 사용자 지시 |
| 2026-09-19 | 사실 | 같은 과목명이 학부마다 다른 학년에 열린다 (경력개발과취업전략 = 로봇 3학년 · 정보융합 4학년 · 소프트웨어 3학년). **`course` 는 학부별로 따로 두는 게 맞다** | 세 학부 대조 |

| 2026-09-20 | 결정 | **폼을 선택식으로 바꾸지 않는다.** 정량 정보는 대부분 팀원이 직접 넣고, Supabase 의 `course` 목록을 보고 표기를 맞춰 적는다. 중복 차단·요청 제한도 넣지 않는다 | 사용자 지시 |
| 2026-09-20 | 결정 | 데이터 품질은 **입력에서 막지 않고 사후 점검으로** 잡는다 — `db/check-reports.sql` 주 1회. `report` 를 고치면 트리거가 `load_event` 를 다시 만들어 집계가 자동으로 맞는다 | 사용자 지시 |
| 2026-09-20 | [정정] | 위 결정으로 **"W3 과목 선택 교체 → W4 QR 배포" 게이트는 폐기.** 09-19에 순서를 못 바꾼다고 적었으나 전제(불특정 다수 입력)가 바뀌었다 | 사용자 지시 |
| 2026-09-20 | 사실 | 트랙 분류 12개·매핑 93건 작성 (`db/seed-tracks.sql`). 로봇 4 / 정보융합 4 / 소프트웨어 4. **[초안]** — 학과 공식 이수체계가 아니라 과목명·배치로 만든 1차안 | Claude 작성, 검토 필요 |
| 2026-09-20 | 노하우 | 과목명 정규화에 `translate` 로 로마숫자를 글자 단위 치환하면 **틀린다** (`자동제어II`→`자동제어11`, `AI수학`→`A1수학`). **문자열 끝에서만** `iv/iii/ii/i` 를 통째로 바꿔야 한다 | 실제 데이터로 재현·수정 |
| 2026-09-20 | 노하우 | 대량 매핑은 `(values ...) as m(...) join track ... join course ...` 형태로 쓴다. 팀원이 SQL 을 몰라도 네 칸짜리 표만 고치면 되고, 없는 과목명은 조용히 빠지므로 확인 쿼리로 잡는다 | `db/seed-tracks.sql` |

| 2026-09-20 | 사실 | **로봇학실험과 AI로봇실험은 사실상 같은 과목.** `로봇학실험/AI로봇실험1~4` 한 과목으로 합쳤다. 로봇학부 35 → **34과목**, 3개 학부 **101과목** | 사용자 확인 |
| 2026-09-20 | 노하우 | `course` 의 유일키가 `(dept_code, name)` 이라 **과목명을 바꾸면 upsert 가 새 행을 만든다.** 이미 시드한 DB 가 있으면 시드 파일 맨 위에 이름 정리 블록(rename → 참조 옮기기 → 삭제)을 넣어야 한다. reset 없이 따라온다 | `db/seed-robot-courses.sql` |
| 2026-09-20 | 노하우 | `report` 를 수정하면 `after insert or update` 트리거가 `load_event` 를 다시 만든다. 과목명 정정·병합 후 집계가 저절로 맞는 근거 | 실제 확인 (56 이벤트 유지) |

| 2026-09-20 | 사실 | 정보융합·소프트웨어는 **강의계획서에 전공선택인데 학과 홈페이지에는 전공필수**인 과목이 있다. 자료 두 개가 어긋난다 | 사용자 진술 |
| 2026-09-20 | 결정 | 전공필수 입력 담당 — **로봇학부는 사용자 본인**, 정보융합·소프트웨어는 **해당 학과 팀원이 직접.** 선후수 이수체계도도 같은 사람이 함께 기입. **지금 당장은 안 한다** | 사용자 지시 |
| 2026-09-20 | 결정 | 개발 3인은 **착수 전에 스택·파일 경계·공용 타입을 합의한다.** Claude 는 앞서 구현하지 않고 제안서만 쓴다 (`docs/dev-stack-proposal.md`) | 사용자 지시 |
| 2026-09-20 | 원칙 | 출력 쪽도 **입력 폼과 같은 스택**(Next.js·TS·Supabase·Vercel)으로 간다. 별도 백엔드·ORM·상태관리·로그인은 넣지 않는다 — 특히 **로그인은 "개인정보 0" 원칙이자 발표 방어 축이라 넣으면 안 된다** | 제안서 |
| 2026-09-22 | 결정 | 참빛설계 주제를 AlohaMini 트레이 서빙 → **사피엔스 휴머노이드 Push-T**로 전환. 같은 랩실 4인 요건 때문에 팀 재구성, 제어 2인/월드모델·RL 2인 분리 수행(교수님 허락). 예산 100만원은 캡스톤용 뎁스카메라 | 사용자 진술 |

| 2026-09-25 | 사실 | **사용자가 기술과경영 과목을 드랍했다** (오래 고민 후 결정). 팀 프로젝트에서 빠지며, 지금까지 만든 것을 팀장에게 인계한다 | 사용자 진술 |
| 2026-09-25 | 미확인 | Supabase 프로젝트·Vercel 프로젝트가 **사용자 개인 계정**에 있다. 인계 시 소유권 이전 또는 팀원 초대가 필요하다 — 처리 여부 미확인 |
| 2026-09-23 | 사실 | 자소서 소재 원천: KT랜선야학 종결보고서(심승유 32회·64h·결석1 / 이호영 24회·48h·결석0, 달성도 80%), 삼성드림클래스 지원서(공군 대대으뜸병사 청소 갈등 해결 — 본인 작성), IC-PBL WRC 3팀(공통2+개별3~4 질문, 20개사, 8/22~24) | `Desktop\광운대학교\대외활동` |
| 2026-09-23 | 미확인 | Claude 채팅 초안의 '모던 로보틱스 매일 2시간·5스프린트·일요일 점검·2일 룰·봉사 주 2시간·건의안 채택'은 로컬 기록에 근거 없음 | 로컬 전사·파일 전수 grep |

| 2026-09-23 | 환경 | **기술과경영(학업나침반) 작업 세션은 로컬 Desktop 세션이 아니라 원격(claude.ai/code) 세션이다.** 데스크톱 세션 도구(`list_sessions`·`unarchive_session`)에 안 잡혀서 Claude가 보관 해제를 못 한다 → **사용자가 claude.ai/code 웹에서 직접 복원** | 보관 포함 목록 50개·전사 검색 3회(학업나침반/학기예보/Supabase)·ListAgents·사이드바 그룹 전부 확인, 로컬엔 STATE.md를 읽은 세션만 있음 |
| 2026-09-03 | 사실 | 손태희장학재단 4기(대학생 성적우수) 서식 3·4·5호를 HWP 바이너리 편집으로 작성 → `Desktop\광운대학교\장학금\손태희장학금\김민기_손태희장학재단_서식3-4-5호.hwp`. 서식 2호·체크리스트는 개인정보 부족으로 미작성 | 로컬 세션 `손태희장학재단 장학금 신청서 작성` |
| 2026-09-03 | 노하우 | 손태희 서식 HWP의 12pt 글자모양 **42번=빨강, 43번=회색**(작성 가이드용). 본문은 **17번(12pt 굴림체, 검정)** 으로 넣어야 한다. 빈 문단은 PARA_TEXT 레코드가 아예 없다 | DocInfo CHAR_SHAPE 색상 파싱 |
| 2026-09-03 | 사실 | CV(2026.08) 기준 **"산업체 인턴 경험 없음"**. 폴더명 `체험형인턴(KETI)` 만 보고 쓴 KETI 인턴 문장은 자소서에서 제거했다. 실제로 했다면 다시 넣을 것 [미확인] | CV 아티팩트 `a57a415e` |
| 2026-09-03 | 사실 | 박수한 교수 = **로봇학부 조교수**. CV의 연구실 표기는 **동적제어연구실**(= DREAM Lab). 평점 4.31/4.4, 26-1학기 18학점 A+ 5·A0 1 | WebSearch(LinkedIn)·CV |

| 2026-09-19 | 선호 | **수업 과제 보고서는 교안 범위 안에서 쓴다.** 초반의 키워드 설정·예시·교안 매칭은 좋지만, 통신·세부 수식 전개처럼 교안에 없는 방향으로 깊게 들어가면 안 된다 (로봇제어 과제2 v1 반려 사유) | 사용자 피드백 |
| 2026-09-19 | 선호 | 보고서에 넣을 이미지는 Claude가 링크(출처·라이선스 포함)를 주고 **사용자가 직접 붙여 넣는다.** 문서에는 `[ 여기에 … 사진 삽입 ]` 자리 + 캡션 + 출처 줄 | 사용자 지시 |
| 2026-09-26 | 선호 | 로봇제어 과제는 **docx만 만든다. PDF는 사용자가 확인 후 직접 내보낸다** | 사용자 지시 |
| 2026-09-26 | 가정 | 로봇제어 과제3-2: pitching·rolling 모두 수평축 회전으로 보고 중력 토크 포함, ROMmax 90°, 5.4 kg = 링크. 교안 94쪽 풀이 필기와 다르면 수정 | Claude 가정, [미확인] |
| 2026-09-26 | 노하우 | 로컬 Windows에 poppler·PyMuPDF가 없어도 **PDF 쪽을 PNG로 렌더링할 수 있다** — PowerShell 5.1에서 WinRT `Windows.Data.Pdf.PdfDocument` + `RenderToStreamAsync`. 쪽마다 별도 프로세스로 호출(`-Pages` 배열 인자는 `-File` 로 넘기면 깨짐). 브라우저 pane의 `file://` PDF는 렌더 타임아웃이 잦다 | `4장 교안_필기.pdf` 검수 |
| 2026-09-26 | 사실 | 로봇제어 `3,4장 필기자료.pdf`(1쪽 손글씨 + 타자 3줄, 2쪽 빈 면)를 타자로 옮겨 `4장 교안_필기.pdf` 로 만듦 — 필기가 있는 19개 슬라이드 아래에 노란 "수업 필기" 띠를 덧붙이는 방식(슬라이드 내용은 안 가림). 판독 불확실 3곳 `[?]`·`[판독 불가]` 표시 | 로컬 세션 |
